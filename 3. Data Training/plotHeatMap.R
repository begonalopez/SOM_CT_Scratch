...
plotHeatMap <- function(som_model, data, variable=0){    
  ...
  require(dummies)
  require(kohonen)
  #source('rstudio/drscratch/coolBlueHotRed.R')
  ...
  while (interactive == TRUE){
    ...
    # plot(som_model, type = "property", property=unit_colors[,2], zlim=c(-3, 3), main=color_by_var, palette.name=coolBlueHotRed)
    # plot(som_model, type = "property", property=unit_colors[,2], zlim=c(0, 1), main="", palette.name=coolBlueHotRed)
